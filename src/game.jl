...
function GI.init(::CubeSpec{N}, state=(board=falses(board_shape(CubeSpec{N})), history=UInt16[])) where N
...
GI.white_reward(g::CubeEnv) = isempty(history(g)) ? 0. : -Float64(sum(x->x^2, history(g)))
...
Base.@propagate_inbounds function action_on_gameactions(σ::AbstractAlgebra.Perm, cids, lids)
    return @inbounds [lids[permutedims(cids[a], σ)] for a in vec(lids)]
end

Base.@propagate_inbounds function action_on_gamestate(
    state,
...
function GI.symmetries(::CubeSpec{N}, state) where N
    ...
    return [ @inbounds action_on_gamestate(state, σ, cids=cids, lids=lids)
...
function GI.render(g::CubeEnv{N}; with_position_names=true, botmargin=true) where N
    ...
    available_actions = GI.actions_mask(g)
    k = ceil(Int, log10(2^N))
    for action in GI.actions(GI,spec(g))
        with_position_names && print(rpad("$action", k+2), " | ")
        color = available_actions[action] ? crayon"bold fg:light_gray" : crayon"fg:dark_gray"
        println(color, GI.action_string(GI.spec(g), action))
    end
    botmargin && print("\n")
end
